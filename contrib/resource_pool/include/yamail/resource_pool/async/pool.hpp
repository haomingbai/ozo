...
public:
    ...
    auto get_auto_waste(io_context_t& io_context, CompletionToken&& token,
        ...
        return asio::async_initiate<CompletionToken, void (boost::system::error_code, handle)>(
            [this, &io_context, wait_duration] (auto&& handler) {
                get(io_context, std::forward<decltype(handler)>(handler), &handle::waste, wait_duration);
            },
            token
        );
    }
    ...
    auto get_auto_recycle(io_context_t& io_context, CompletionToken&& token,
        ...
        return asio::async_initiate<CompletionToken, void (boost::system::error_code, handle)>(
            [this, &io_context, wait_duration] (auto&& handler) {
                get(io_context, std::forward<decltype(handler)>(handler), &handle::recycle, wait_duration);
            },
            token
        );
    }
